...
| `analog_output_raw` | `uint16` | 2 | DAC1、DAC2 当前输出原始值，范围 0 - 4095 |

当前固件的默认周期为 100 ms。协议允许在链路拥塞时跳过遥测帧，事件和 ACK 使用独立路径。载荷长度为 52 字节，`analog_output_raw` 用于让上位机回读模拟输出，而不是只相信设定值。
...
例外：`TELEMETRY` 载荷从 48 字节扩展为 52 字节以携带 `analog_output_raw`。STM32 和
ESP32-S3 必须使用同一提交的固件一起烧录，混用旧固件会导致遥测帧长度校验失败并被丢弃。

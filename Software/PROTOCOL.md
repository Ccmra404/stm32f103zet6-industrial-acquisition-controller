...
| `0x11` | `EVENT` | STM32 到 ESP32-S3 | 上报边沿、报警和故障 |
...
默认周期为 50 ms。链路拥塞时允许跳过遥测帧，不允许跳过事件和 ACK。
...
| `0x0100` | 开始模拟量校准 | `argument0` 为通道，`argument1` 为校准点 |
| `0x0101` | 开始温度校准 | `argument0` 为参考温度 |
| `0x0200` | 保存配置 | 参数保留 |
...
ESP32-S3 发出命令后等待 500 ms。超时后复用原 `request_id` 重试，最多重试两次。STM32 保存最近 16 个请求结果，收到重复请求时返回原结果，不再次执行输出。

ESP32-S3 收到 `OK` 后更新界面。收到错误后显示错误原因，不自行假定命令成功。
